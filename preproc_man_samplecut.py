...
params = load_gamma_params("phones_length_gamma_man.param")
...
# Random Sampling Cut
def open_and_cut_phones_random_sampling(wave_path, wave_name, filtered_df, params, no_real_cut=False): 
    sp = Sound_Proc()
    ...
    # Framify
    # Create a dictionary with the three lists as values and the column names as keys
    data = {'rec': wave_name, "idx": list(map("{:08d}".format, range(len(c_duration)))), 'start_time': cst, 'end_time': cet, 'token': "", 'duration': c_duration, 'n_frames':cut_n_frames}
    # Create a Pandas DataFrame from the dictionary
    df = pd.DataFrame(data)
...
def collaboration_single_work(my_work_pool, fun, my_wave_dir, my_anno_dir, my_save_dir, my_log_dir, my_params, no_real_cut): 
    ...
    filtered_df = filter_tokens_and_get_df(os.path.join(my_anno_dir, "log.csv"), keepSIL=False)
...
def run(domain=None, no_real_cut=False): 
    ...
    if domain == "phone-random": 
        ...
        mpm = MultiprocessManager(open_and_cut_phones_random_sampling, 
                                as_wav_path, as_phones_extract_path, 
                                as_phone_seg_random_path, 
                                as_phone_seg_random_log_path, 
                                params, num_workers=n_worker)
        
        mpm.collaboration_work()

        #### Bind csvs into one
        csv_bind(as_phone_seg_random_log_path)
    elif domain == "phone-anno": 
        # aligned cutting
        mpm = MultiprocessManager(open_and_cut, 
                                as_wav_path, 
                                as_phones_extract_path, 
                                as_phone_seg_anno_path, 
                                as_phone_seg_anno_log_path, 
        ...
        #### Bind csvs into one
        csv_bind(as_phone_seg_anno_log_path)
    
    elif domain == "phone-random-bind": 
        csv_bind(as_phone_seg_random_log_path)
    
    elif domain == "phone-anno-bind":
        csv_bind(as_phone_seg_anno_log_path)
